...
impl LintRule for UnusedSource {
    fn code(&self) -> &'static str { "W003" }

    fn check(&self, style: &Style) -> Vec<Diagnostic> {
        let used: std::collections::HashSet<&str> = style.layers.iter()
            .filter_map(|l| l.source.as_deref())
            .collect();

        style.sources.keys()
            .filter(|id| !used.contains(id.as_str()))
            .map(|id| Diagnostic::warning(
                "W003",
                format!("sources.{}", id),
...
mod tests {
    ...
    #[test]
    fn test_used_source() {
        let style = parse(r#"{"version":8,"sources":{"s":{"type":"geojson","data":null}},"layers":[{"id":"l","type":"fill","source":"s"}]}"#);
