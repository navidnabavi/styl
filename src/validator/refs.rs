...
pub fn validate_refs(style: &Style) -> Vec<Diagnostic> {
    ...
    match &style.sprite {
        ...
        Some(SpriteValue::Array(entries)) => {
            if entries.is_empty() {
                diags.push(Diagnostic::error("E008", "sprite", "sprite array must not be empty"));
            }
            for (i, entry) in entries.iter().enumerate() {
                if entry.id.is_empty() {
                    diags.push(Diagnostic::error("E008", format!("sprite[{}].id", i), "sprite entry id must be non-empty"));
                }
                if entry.url.is_empty() {
                    diags.push(Diagnostic::error("E008", format!("sprite[{}].url", i), "sprite entry url must be non-empty"));
                }
            }
        }
        _ => {}
...
mod tests {
    ...
    fn test_sprite_string_valid() {
        let style = parse(r#"{"version":8,"sprite":"https://example.com/sprite","sources":{},"layers":[]}"#);
        assert!(validate_refs(&style).iter().all(|d| d.code != "E008"));
    }

    #[test]
    fn test_sprite_array_valid() {
        let style = parse(r#"{"version":8,"sprite":[{"id":"default","url":"https://example.com/sprite"}],"sources":{},"layers":[]}"#);
        assert!(validate_refs(&style).iter().all(|d| d.code != "E008"));
    }
    ...
    fn test_sprite_array_empty_id() {
        let style = parse(r#"{"version":8,"sprite":[{"id":"","url":"https://example.com/sprite"}],"sources":{},"layers":[]}"#);
        let diags = validate_refs(&style);
        assert!(diags.iter().any(|d| d.code == "E008" && d.path.contains("sprite[0].id")));
    }
}
