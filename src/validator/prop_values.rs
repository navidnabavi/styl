...
pub fn get_prop_type(name: &str) -> Option<PropType> {
    match name {
        ...
        "icon-anchor" => Some(PropType::Enum(&[
            "center",
            "left",
            "right",
            "top",
        ...
        "raster-saturation" => Some(PropType::Number {
            min: -1.0,
            max: 1.0,
        }),
        "raster-contrast" => Some(PropType::Number {
            min: -1.0,
            max: 1.0,
        }),
        ...
        // --- Hillshade paint ---
        "hillshade-illumination-direction" => Some(PropType::Number {
            min: 0.0,
            max: 359.0,
        }),
        ...
        "sky-gradient-radius" => Some(PropType::Number {
            min: 0.0,
            max: 180.0,
        }),
...
pub fn validate_prop_value(name: &str, value: &Value, path: &str) -> Vec<Diagnostic> {
    ...
    match prop_type {
        ...
        PropType::Enum(variants) => match value.as_str() {
        ...
        PropType::Number { min, max } => match value.as_f64() {
            Some(n) if n >= min && n <= max => vec![],
            Some(n) => vec![Diagnostic::error(
                "E018",
                path,
                format!(
                    "\"{}\" expects number in [{}, {}], got {}",
                    name, min, max, n
                ),
            )],
            None => vec![Diagnostic::error(
        ...
        PropType::NumberMin { min } => match value.as_f64() {
            Some(n) if n >= min => vec![],
            Some(n) => vec![Diagnostic::error(
                "E018",
                path,
        ...
        PropType::Color => match value.as_str() {
            Some(s) => {
            ...
            None => vec![Diagnostic::error(
                ...
                path,
                format!(
                    "\"{}\" expects a CSS color string, got {}",
                    name,
                    value_type_name(value)
                ),
            )],
...
mod tests {
    ...
    fn test_opacity_in_range_valid() {
        let diags =
            validate_prop_value("fill-opacity", &json!(0.5), "layers[0].paint.fill-opacity");
    ...
    fn test_opacity_out_of_range() {
        let diags =
            validate_prop_value("fill-opacity", &json!(1.5), "layers[0].paint.fill-opacity");
        assert!(diags
            .iter()
            .any(|d| d.code == "E018" && d.message.contains("fill-opacity")));
    }
    ...
    fn test_enum_invalid() {
        ...
        assert!(diags
            .iter()
            .any(|d| d.code == "E018" && d.message.contains("\"butt\", \"round\", or \"square\"")));
    }
    ...
    fn test_color_invalid_string() {
        ...
        assert!(diags
            .iter()
            .any(|d| d.code == "E018" && d.message.contains("fill-color")));
    }
    ...
    fn test_number_min_negative() {
        ...
        assert!(diags
            .iter()
            .any(|d| d.code == "E018" && d.message.contains("line-width")));
    }
    ...
    fn test_visibility_invalid() {
        ...
        assert!(diags
            .iter()
            .any(|d| d.code == "E018" && d.message.contains("\"visible\" or \"none\"")));
    }
    ...
    fn test_array_valid() {
        let diags = validate_prop_value(
            "text-font",
            &json!(["Open Sans Regular", "Arial Unicode MS"]),
            "layers[0].layout.text-font",
        );
        assert!(diags.is_empty());
    }
    ...
    fn test_array_wrong_type() {
        let diags = validate_prop_value(
            "text-font",
            &json!("Open Sans Regular"),
            "layers[0].layout.text-font",
        );
        assert!(diags
            .iter()
            .any(|d| d.code == "E018" && d.message.contains("text-font")));
    }
    ...
    fn test_array_string_first_element_not_expression() {
        ...
        let diags = validate_prop_value(
            ...
            &json!(["center", "left"]),
            "layers[0].layout.text-variable-anchor",
        );
